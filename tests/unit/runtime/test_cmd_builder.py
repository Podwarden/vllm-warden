...
def test_build_vllm_args_bind_host_overridable(monkeypatch):
    # The docker driver runs the engine as a SEPARATE container, so the
    # published port only works if vLLM binds 0.0.0.0. VW_ENGINE_BIND_HOST
    # MUST flow through to --host (read per-call, no module reimport).
    monkeypatch.setenv("VW_ENGINE_BIND_HOST", "0.0.0.0")
    args = build_vllm_args(_row(), port=10001)
