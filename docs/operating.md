...
| `VW_REQUEST_MAX_WALL_S` | `0.0` (off) | no | Server-side wall-clock backstop for a single proxied request. `0.0` disables it (behaviour-identical to pre-reaper). When `> 0`, a streamed generation running longer than this many seconds is torn down — the proxy stops draining upstream, `aclose()`s the upstream vLLM socket (so the engine aborts and frees KV blocks) and releases the scheduler slot. Guards against an abandoned-but-transport-alive client pinning a slot indefinitely (the client keeps draining tokens, so no `http.disconnect` reaches uvicorn and Starlette never cancels the stream). Recommended production value: `600` (match the upstream harness `request_timeout_seconds`). |
